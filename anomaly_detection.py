...
def l2_norm(x, y, axis=None):
    if axis is None:
        return tf.reduce_sum(tf.pow(x-y, 2)) # pow求幂次方
    else:
...
def MSE(x, y, axis=None):
    if axis is None:
        return tf.reduce_mean(tf.pow(x-y, 2)) # mean求tensor中平均值
    else:
...
def Encoder(inputs, is_training, dim=DIM, z_dim=ZDIM, rand_sampling='normal', reuse=None, z_reg_type=None, denoise=None):
    with tf.variable_scope('Encoder', reuse=reuse):
        if denoise is not None:
            inputs = tf.nn.dropout(inputs, keep_prob=denoise) # 为了防止过拟合而使用的函数，就是在不同的训练过程中随机扔掉一部分神经元，keep_prob为每个元素被保留下来的概率，
        output = tf.reshape(inputs, [-1, 1, 64, 64]) # 改变tensor的形状，-1，表示自动计算此维度。
        output = lib.ops.conv2d.Conv2D('Encoder.Input', 1, dim, 3, output, he_init=False)# 卷积操作
    ...
    if z_reg_type is None:
        return output
    elif z_reg_type == 'tanh_fc':# DEFAULT (paper version)
    ...
    with tf.Session(config=tf.ConfigProto(device_count={'GPU': len(DEVICES)}, allow_soft_placement=True)) as session:
        ...
        for is_anom, _gen in enumerate([test_gen(), ano_gen()]):
            ...
            for _idx in xrange(nr_mapping_batches):
                (_data,) = _gen.next()
                _img_dist, _dist_z, _z = session.run([img_distance, z_distance, emb_query], feed_dict={real_data: _data})

                if np.mod(_idx+1,100)==0:
                    print ("%d (of %d) imgs processed ..\t img_m=%.4f\tz_m=%.4f" %((_idx+1)*BATCH_SIZE, nr_mapping_batches*BATCH_SIZE, _img_dist.mean(), _dist_z.mean()))
        ...
        with open(log_meta_path.replace('.csv', '.pkl'), 'w') as f:
           cPickle.dump({
                        'encodings': encodings,
                        'img_dists': img_dists,
                        'z_dists': z_dists, }, f, cPickle.HIGHEST_PROTOCOL)
...
if __name__ == '__main__':
    # anomaly_scoring("path-to-'checkpoints'-folder-of-encoder-training"s, <checkpoint-iter>) <checkpoint-iter>是指迭代次数
    anomaly_scoring("./z_encoding_d/izi_tanh_fc/wganTrain-iter7/wgangp_64x64_normal_crIt5_2019-06-25-1526-enc_iter50000_2019-06-25-2234/z_encoding_izif-MSE-k1.00_tanh_fc/checkpoints", 20000)